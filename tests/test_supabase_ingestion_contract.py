...
class SupabaseIngestionContractTests(unittest.TestCase):
    ...
    def test_edge_function_validates_physical_espresso_bounds(self) -> None:
        ...
        self.assertIn("MIN_VALID_EPOCH = 1_600_000_000", source)
        self.assertNotIn("MAX_UNIX_EPOCH", source)
        self.assertNotIn("2_147_483_647", source)
        self.assertIn("requireTimestamp(payload, 'timestamp'", source)
        self.assertIn("validateRecommendationTimestampOrder(payload, errors)", source)
        self.assertIn("updated_at cannot precede created_at", source)
        self.assertIn("is outside the recommendation lifecycle", source)
        self.assertNotIn("Number.MAX_SAFE_INTEGER", source)
