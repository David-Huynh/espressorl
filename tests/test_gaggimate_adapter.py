...
class GaggimateAdapterTests(unittest.TestCase):
    ...
    def test_optimizer_settings_payload_translates_to_canonical_event(self) -> None:
        ...
        self.assertEqual(event.taste_objective, {"mode": "custom", "sweet": "high"})
    ...
    def test_shot_profile_payload_trims_post_finish_inactive_control_tail(self) -> None:
        client = GaggimateMQTTClient(
            config=Config(mqtt_host="localhost", data_dir=Path("/tmp")),
            on_shot=lambda event: None,
            on_feedback=lambda event: None,
        ...
        event = client.translate_shot_payload(
            {
                ...
                "target_pressure": [0, 4, 9, 0, 0],
                "flow": [0, 1, 2, 99, 99],
                "pump_flow": [0, 1.2, 2.4, 0, 0],
                "target_flow": [0, 0, 0, 0, 0],
                "temperature": [86.0, 86.2, 86.5, 86.5, 86.5],
                "target_temperature": [86.5, 86.5, 86.5, 86.5, 86.5],
                "pump_target_mode": [0, 1, 1, 0, 0],
                "valve_open": [False, True, True, False, False],
                "weight": [0, 8, 36, 70, 120],
                "target_yield_g": 36.0,
                "beverage_out_g": 120.0,
    ...
    def test_shot_profile_payload_does_not_trim_targetless_valve_open_phase(self) -> None:
        ...
        event = client.translate_shot_payload(
            {
                ...
                "pressure": [0, 1, 1, 1],
                "target_pressure": [0, 0, 0, 0],
                "flow": [0, 0, 0, 0],
                "pump_flow": [0, 0, 0, 0],
                "target_flow": [0, 0, 0, 0],
                "pump_target_mode": [0, 0, 0, 0],
                "valve_open": [False, True, True, True],
                "weight": [0, 1, 2, 3],
                "target_yield_g": 36.0,
                "beverage_out_g": 3.0,
            },
        ...
        self.assertEqual(event.time_ms, [0, 250, 500, 750])
        self.assertEqual(event.weight, [0, 1, 2, 3])
        self.assertEqual(event.beverage_out_g, 3.0)
